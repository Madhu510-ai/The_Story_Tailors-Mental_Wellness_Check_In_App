...
function runImport() {
  ...
  if (!excelPath) {
    ...
    console.log("Please place your questionnaire spreadsheet file in: public/dashboard/data/");
    console.log("Or run: node scripts/import_questionnaire.cjs <path-to-file.xlsx>");
    process.exit(1);
  }
  ...
  rows.forEach((row, rowIdx) => {
    ...
    const scene =
      getVal("Scene", "Scenario", "Description") || `A situation arises in ${storyTitle}.`;
    ...
    let storyObj = genreObj.stories.find(
      (s) => s.id === storyId || normalizeKey(s.title) === storyId,
    );
    if (!storyObj) {
      storyObj = {
        id: storyId,
  ...
  console.log(`\n✅ Successfully imported spreadsheet questions to:\n   ${STORIES_JSON_PATH}\n`);
  console.log(`📊 Import Summary: ${updatedGenres.length} Genre(s)`);
