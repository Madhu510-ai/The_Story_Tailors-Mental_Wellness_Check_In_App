...
const QUESTIONNAIRE_DIR = path.join(
  __dirname,
  "..",
  "public",
  "dashboard",
...
function processAllQuestionnaires() {
  ...
  files.forEach((fileName, fIdx) => {
    ...
    let genreName = "";
    let genreId = fileName
      .replace(/(_psychology_scored|_unique_final|_story_specific)+/gi, "")
      .replace(/\.(xlsx|xls)$/i, "")
      .toLowerCase();
    ...
    wb.SheetNames.forEach((sheetName) => {
      if (sheetName.toUpperCase() === "INDEX" || sheetName.toUpperCase() === "SCORING_METHOD")
        return;
      ...
      rows.forEach((row, rIdx) => {
        const qId =
          row["Question ID"] || `${genreId.toUpperCase()}_Q${String(rIdx + 1).padStart(3, "0")}`;
        const beat = row["Story Beat"]
          ? `Beat ${row["Story Beat"]}`
          : `Beat ${Math.floor(rIdx / 6) + 1}`;
  ...
  // Write modular genre files under public/dashboard/data/genres/
  ...
  // Write lightweight index file (genres.json) containing genre list & story metadata
  const genresIndex = {
    genres: genresList.map((g) => ({
      ...
      stories: g.stories.map((s) => ({
        ...
        dataUrl: `data/stories/${g.id}/${s.id}.json`,
      })),
    })),
  };
  ...
  console.log(`Saved index to: ${OUTPUT_JSON_PATH} and ${INDEX_PATH}`);
